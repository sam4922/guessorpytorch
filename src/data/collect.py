...
class CoverageCollector:
    ...
    def _fallback_random_tiles(self, zoom_level: int, bounds: Dict) -> List[Tuple[int, int, int]]:
        ...
        # Limit to a small number for testing (since we're not using coverage API)
        max_tiles = min(10, len(tiles))  # Reduced from 50 to 10
...
class ImageCollector:
    ...
    def collect_images_from_coordinates(self, coordinates: List[Tuple[float, float]], 
        ...
        """
        Collect Street View panoramas from a list of coordinates.
        
        Args:
            coordinates: List of (lat, lng) coordinate pairs
            max_images: Maximum number of images to collect (uses config if None)
            
        Returns:
            Dictionary with collection statistics
        """
        if max_images is None:
            max_images = self.config['data_collection']['images_per_session'] // len(self.config['data_collection']['headings'])
        ...
        pbar = tqdm(total=max_images, desc="Collecting panoramas")
        
        try:
            for lat, lng in shuffled_coords:
                if collected >= max_images:
                    break
                
                # Try to get Street View panorama (all headings)
                images_data = self.api.get_streetview_panorama(lat, lng)
                
                if images_data:  # All headings succeeded
    ...
    def collect_images_from_tiles(self, covered_tiles: List[Tuple[int, int, int]], 
        ...
        Returns:
            Dictionary with collection statistics
        """
        print(f"Collecting images from {len(covered_tiles)} tiles")
        
        # Sample coordinates from tiles, but limit total coordinates to max_panoramas * 2
        # This prevents generating way more coordinates than we need
        if max_panoramas:
            target_coords = max_panoramas * 2  # Generate 2x coordinates as buffer
            samples_per_tile = max(1, target_coords // len(covered_tiles))
        ...
        for x, y, zoom in covered_tiles:
            ...
            if max_panoramas and len(all_coordinates) >= max_panoramas * 2:
                break
...
def main(max_images: int = None, bounds: Dict = None):
    ...
    try:
        # Convert max_images to max_panoramas
        max_panoramas = None
        if max_images:
            headings_count = len(json.load(open("src/config.json"))['data_collection']['headings'])
            max_panoramas = max(1, max_images // headings_count)
